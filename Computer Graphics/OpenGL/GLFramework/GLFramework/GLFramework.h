...
private:
	...
	void	(*m_pfMouseInput)(int button, int state, int x, int y)	{ nullptr } ;

	CScene* m_pCurrentScene;
	CScene* m_pSceneArr[CScene::Scenelist::Count];
...
public:
	...
	void SetMouseInputFunc	(void(*func)(int, int, int, int)) { m_pfMouseInput = func; }
	
	void BuildSceneArr();
	void ChangeScene(CScene::Scenelist tag);
};
